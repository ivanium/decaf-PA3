...
public class Translater {
  ...
  public Temp genFuncForDcopy(Class c, Temp loc) {
    // currentFuncty = new Functy();
    // currentFuncty.label = Label.createLabel("_" + c.getName() + "_" + "Dcopy", true);
    // currentFuncty.paramMemo = Tac.genMemo("dcopy for " + c.getName());
    // genMark(currentFuncty.label);
    Temp size = genLoadImm4(c.getSize());
    genParm(size);
    Temp newObj = genIntrinsicCall(Intrinsic.ALLOCATE);
    ...
    // Temp loc;
    ClassScope cs = c.getAssociatedScope();
    Iterator<Symbol> iter = cs.iterator();
    while (iter.hasNext()) {
      Symbol sym = iter.next();
      if (sym.isVariable()) {
        int offset = OffsetCounter.WORD_SIZE*(sym.getOrder()+1);
        if (sym.getType().isClassType()) {
          Temp subClassLoc = genAdd(loc, genLoadImm4(offset));
          genStore(genFuncForDcopy(((ClassType)sym.getType()).getSymbol(), subClassLoc), newObj, offset);
        } else {
          genStore(genLoad(loc, offset), newObj, offset);
        }
      }
    }
    genStore(genLoadVTable(c.getVtable()), newObj, 0);
